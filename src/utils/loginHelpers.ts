 ...
 */
export const validateLoginForm = (email: string, password: string): { isValid: boolean, error: string | null } => {
 ...
 * @param email - User email
 */
export const recordFailedLoginAttempt = (email: string): void => {
    const now = Date.now();
 ...
 * @param email - User email
 * @returns { allowed: boolean, error: string|null, remaining?: number, retryAfter?: number }
 */
export const checkLoginRateLimit = (email: string): LoginRateLimitResult => {
    const now = Date.now();
    const key = email.toLowerCase().trim();
    const record = loginAttemptsStore.get(key);
